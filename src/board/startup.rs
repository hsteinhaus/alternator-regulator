...
use esp_hal::{clock::CpuClock, delay::Delay, dma::{DmaRxBuf, DmaTxBuf}, dma_buffers, gpio::{Input, InputConfig, Level, Output, OutputConfig, Pull}, i2c::master::{BusTimeout, Config as I2cConfig, I2c}, rng::Rng, spi::master::{Config as SpiConfig, Spi}, spi::Mode, system::CpuControl, time::Rate, timer::{timg::TimerGroup, AnyTimer}, Async};
...
impl Resources {
    pub fn initialize() -> Self {
        ...
        let cs_display = Output::new(peripherals.GPIO14, Level::High, OutputConfig::default());
        let shared_spi_bus = make_static!(RefCell::new(spi_bus));
        let spi_device = RefCellDevice::new(shared_spi_bus, cs_display, Delay::new()).unwrap();
